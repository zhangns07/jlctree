...
for (sim in c(FLAGS$minsim:FLAGS$maxsim)){
    ...
    if (FLAGS$alg == 'jlcmm'){
        ...
        if(FLAGS$inter){ classmb <- ~X1*X2+X3+X4+X5+X6+X7+X8+X9+X10 
        ...
        for(ng in c(2:6)){
            ...
            Rfilename <- paste0(FLAGS$outdir,'/simret_extra_RData/',Rbasefilename,'_ng_',ng,'_sim_',sim,'.RData')
        ...
        if (FLAGS$stop_thre==-1){
            ...
            cond_ind_tree <- rpart(model.frame
                                   (cbind(time_L,time_Y,delta,y,X3,X4,X5)~X1+X2+X3+X4+X5+X6+X7+X8+X9+X10, data = data), 
            ...
            Rfilename <- paste0(FLAGS$outdir,'/simret_extra_RData/',Rbasefilename,'_sim_',sim,'.RData')
    ...
    filename <- paste0(FLAGS$outdir,'/simret_extra/',RETbasefilename,'.csv')
