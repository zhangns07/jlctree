...
opt2 <- FLAGS; opt2$help <- NULL; opt2$outdir<- NULL; opt2$model <- 'model32'; opt2$alg<-'lcmm'
RETbasefilename <- paste0(paste0(names(opt2),"_",opt2),collapse="_")

opt3 <- FLAGS; opt3$help <- NULL; opt3$outdir<- NULL; opt3$model <- 'model32'; opt3$alg<-'lcmm'; opt3$minsim<-NULL; opt3$maxsim<-NULL
...
for (sim in c(FLAGS$minsim:FLAGS$maxsim)){
    ...
    for(ng in c(2:6)){
        ...
        Rfilename <- paste0(FLAGS$outdir,'/simret_model32_RData/',Rbasefilename,'_ng_',ng,'_sim_',sim,'.RData')
    ...
    filename <- paste0(FLAGS$outdir,'/simret_model32/',RETbasefilename,'.csv')
