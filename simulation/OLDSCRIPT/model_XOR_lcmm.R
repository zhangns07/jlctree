...
option_list <- list(
                    make_option(c("-N", "--Nsub"), type="numeric", default=200, help=""),
...
opt2 <- FLAGS; opt2$help <- NULL; opt2$outdir<- NULL; opt2$model <- 'model34'; opt2$alg<-'lcmm'
RETbasefilename <- paste0(paste0(names(opt2),"_",opt2),collapse="_")

opt3 <- FLAGS; opt3$help <- NULL; opt3$outdir<- NULL; opt3$model <- 'model34'; opt3$alg<-'lcmm'; opt3$minsim<-NULL; opt3$maxsim<-NULL
...
#    g <- 2*(X1)+X2+1
    g <- as.numeric((X1-0.5)*(X2-0.5)>0)*3+1
    ...
    BICs2 <- rep(Inf, 6)
    initB2 <- rep(1,6)#1:B=m1; 2:B=NULL; 3:B=random(m1); 4:none worked
    tik <- Sys.time()
    for(ng in c(2:6)){
        mod <- Jointlcmm(fixed=y~X1+X2+X3+X4+X5,mixture=~1,
                        classmb=~X1+X2+X3+X4+X5,subject='ID',
        ...
        assign(paste0('m2',ng), mod)
    }
    tok <- Sys.time()

    best_ng <- which.min(BICs)
    mod1 <- get(paste0('m',best_ng))
    best_ng <- which.min(BICs2)
    mod2 <- get(paste0('m2',best_ng))

    eval_lcmm_pred(data,dist,slopes,parms,mod1,pseudo_g)
    eval_lcmm_pred(data,dist,slopes,parms,mod2,pseudo_g)

    table(g,mod1$pprob$class)
    table(g,mod2$pprob$class)

    chisq.test(g,mod1$pprob$class)
    chisq.test(g,mod2$pprob$class)
