...
for (sim in c(FLAGS$minsim:FLAGS$maxsim)){
    ...
    if (FLAGS$alg == 'jlcmm'){
	    currRET <- RET[RET_iter,]
		    ...
		    tmpeval <- eval_lcmm_pred(data,dist,slopes,parms,mod,g=pseudo_g)
		...
		tmpeval <- eval_tree_pred(data,dist, slopes, parms, rep(1,nrow(data)),g=pseudo_g)
            RET[RET_iter,'ISE'] <- tmpeval['ISE']
            RET_iter <- RET_iter+1
        } else {
	    ...
	    tmpeval <- eval_tree_pred(data,dist, slopes, parms, cond_ind_tree$where,g=pseudo_g)
            RET[RET_iter,'ISE'] <- tmpeval['ISE']
	    RET_iter <- RET_iter+1
