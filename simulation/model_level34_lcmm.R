...
for (sim in c(FLAGS$minsim:FLAGS$maxsim)){
    ...
    for(ng in c(2:6)){
        ...
        Rfilename <- paste0(FLAGS$outdir,'/simret_model34_RData/',Rbasefilename,'_ng_',ng,'_sim_',sim,'.RData')
    ...
    filename <- paste0(FLAGS$outdir,'/simret_model34/',RETbasefilename,'.csv')
