...
for(treemodel in c('inv','var','var_var1')){
    for (STOP in c(2.71, 3.84, 6.63)){
        resultdir <- paste0('RESULTS_',nfolds,'folds_stop_',STOP,'/paquid_tree_',treemodel,'_moretimevar')
...
treemodel <- 'var'
treemodel <- 'var_var1'
folder  <- paste0('RESULTS_',nfolds,'folds','_stop_6.63/paquid_tree_',treemodel,'_moretimevar')
folder  <- paste0('RESULTS_',nfolds,'folds','/paquid_tree_',treemodel,'_moretimevar')
folder  <- paste0('RESULTS_',nfolds,'folds','_stop_2.71/paquid_tree_',treemodel,'_moretimevar')
...
tree_mid <- jlctree(survival=Surv(tstart, tstop, death)~CEP+male+
                ...
                subject='id',data=data,
                control=list(cp=-Inf),
                parms=list(min.nevents=5,stop.thres=3.84,stable=TRUE))
tree_mid$tree
